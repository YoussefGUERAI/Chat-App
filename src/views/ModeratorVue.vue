<template>
    <div class="moderator-dashboard">
        <!--nav bar-->
        <mini-navbar :currentUser="currentUser" class="column"></mini-navbar>
            ...
            >
                ...
                <div class="d-flex align-items-center g-2">
                    <router-link :to="`/profile/${user.id}`"
                        ><input type="button" value="go to" class="goto"
                    /></router-link>
                    <input
                        v-if="!(user.id === auth.currentUser?.uid)"
                        type="button"
                        :value="user.role === 'banned' ? 'unban' : 'ban'"
        ...
        <div class="column">
            ...
            >
                ...
                <router-link :to="`/group-profile/${group.id}`"
                    ><input type="button" value="go to" class="goto"
                /></router-link>
                <!--go to group page-->
                ...
                <div class="report-info">
                    <div v-if="report.targetType === 'user'">
                        <img
                            :src="
                                report.target?.pfp ||
                                require('@/assets/pfp_default.jpg')
                            "
                            alt="User Profile Picture"
                            class="user-pfp"
                        />
                    ...
                    <div class="d-flex align-items-center g-2">
                        <router-link
                            ...
                            :to="`/profile/${report.target.id}`"
                            ><input type="button" value="go to"
                        /></router-link>
                        <router-link
                            v-if="report.targetType === 'group'"
                            :to="`/group-profile/${report.target.id}`"
                            ><input type="button" value="go to"
                        /></router-link>
    ...
    return reportsList.value.filter((report) => {
        ...
        if (reportSearch) {
            if (report.targetType === "user") {
                match = report.target?.username
                    ?.toLowerCase()
                    .includes(reportSearch);
            } else {
                match = report.target?.name
                    ?.toLowerCase()
                    .includes(reportSearch);
            }
        }
