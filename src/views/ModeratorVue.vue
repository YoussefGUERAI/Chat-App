    ...
    <div class="moderator-dashboard">
      ...
      <div class="column">
        ...
        <div v-for="user in filteredUserList" :key="user.id" class="user-info">
          ...
          <p>{{ user.username }}</p>
          <p>{{ user.email }}</p>
          <router-link :to="`/profile/${user.id}`"><input placeholder="go to" /></router-link>
          <input v-if="!(user.id === auth.currentUser?.uid)" type="button" :value="user.role === 'banned' ? 'unban' : 'ban'" 
      ...
      <div class="column">
        ...
        <div v-for="group in filteredGroups" :key="group.id" class="group-item">
          ...
          <router-link :to="`/group/${group.id}`"><input placeholder="go to" /></router-link> <!--go to group page-->

          <input placeholder="delete group" @click="deleteGroup(group.id)" />
        </div>
      </div>
      ...
      <div class="column">
        ...
        <div v-for="report in filteredReportList" :key="report.id" class="report-card">
          <div class="report-info">
            ...
            <input v-if="report.targetType === 'group'" placeholder="delete group" @click="deleteGroup(report.target.id)" />
...
const filteredGroups = computed( () => {
    ...
    return groupList.value.filter(group => {
        ...
        return !groupSearch || grouptitle.includes(groupSearch)
    })
})
...
const filteredReportList = computed ( () => {
    ...
    return reportsList.value.filter(report => {
        const selectTitle = reportSearch;
        const selectCategory = reportCategory;
    
        return (!selectTitle || report.title.trim().toLowerCase().includes(selectTitle)) && 
               (selectCategory === 'All' || selectCategory === report.targetType)
    })
...
.moderator-dashboard {
  display: flex;
  padding: 20px;
  padding-left: 100px; /* ← this creates space for the fixed navbar */
  gap: 24px;
  overflow-x: auto;
}
...
.moderator-dashboard .mini-navbar {
  ...
  left: 0;
  height: 100vh;
  width: 80px; /* or whatever fits your nav */
